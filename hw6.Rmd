...
**Also need a fast helper function to join these together. Maybe mcapply or something? I just have a simple wrapper for now. Sorting the columns too for faster binding later.**
...
**Currently only running on part of the data. I think we have memory problems if we try to do it all at once.**

```{r}
final_predictions = adply(test_set[300:310,], 1, function(x) prediction_wrapper(x, cbs_dt, features, all_stops, ordered_names),
                           .progress = "text")
final_predictions
```
...
```{r}
 prediction_wrapper(test_set[307,], cbs_dt, features, all_stops, ordered_names)
```

Looks like there are no similar rows! Need to add an exception to use some set of default probabilities in this case. That should be a quick fix.

```{r}
get_similar_rows(test_set[307,], cbs_dt)
```
