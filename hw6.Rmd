...
```{r}
weat_cond %>% group_by(year,month,day,hour) %>% summarise(n=n()) %>% filter(n>1)
```


```{r}
cbs_dt_wet = merge(cbs_dt,weat_cond,all.x=T,sort = F)
```
...
get_similar_rows <- function(row, input_dt, duration_delta = 0.2,
                             month_delta = 0, hour_delta=1, temp_delta = 5) {
...
test_set = test_set %>% mutate(year = lubridate::year(start_date),
                               ...
                               # wday returns 2~6 for Monday~Friday
                               weekday = lubridate::wday(start_date) %in% 2:6)
test_set_wet = merge(test_set,weat_cond,all.x = T,sort = F)
```
...
```{r}
final_predictions = adply(test_set, 1, function(x) prediction_wrapper(x, cbs_dt, features, all_stops, ordered_names),
                           .progress = "text") %>% select(-exclude_features)
final_predictions
```
