...
```{r}
#remove start_date, start_station_number, end_station_number, end_date
cbs_dt <- cbs_dt%>% select(-c(start_date, end_date, start_station_number, end_station_number)) 
#change all categorical data into numeric
sparse_matrix <- sparse.model.matrix(end_station ~ ., data = cbs_dt)[,-1]
head(sparse_matrix)
#convert output as numeric vector 
end_station = factor(cbs_dt$end_station)
label = as.integer(end_station)-1
bst <- xgboost(data = sparse_matrix, label = label, max_depth = 4,
...
similar1 <- get_similar_rows(cbs_dt[1,])
```
...
test_set = test_set %>% mutate(year = lubridate::year(start_date),
                     month = lubridate::month(start_date),
                     day = lubridate::day(start_date),
                     hour = lubridate::hour(start_date),
                     # wday returns 2~6 for Monday~Friday
                     weekday = lubridate::wday(start_date) %in% 2:6)
test_wet = merge(test_set,weat_cond,all.x = T,sort = F) %>% arrange(start_date)
