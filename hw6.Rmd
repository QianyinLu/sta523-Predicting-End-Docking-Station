...
```{r}
cbs_dated = cbs_collected %>% 
  mutate(time=start_date %>% format("%Y-%m-%d-%H-%w")) %>% 
    separate(time,c("year","month","day","hour","weekday"),sep="-")
```
...
```{r}
for(i in 8:20){
  plot<-ggplot()+geom_histogram(aes(x=weather[,i]))+
    ggtitle(colnames(weather)[i])
  print(plot)
}
```
...
```{r}
for(i in c(6,7,21:23)){
  plot<-ggplot()+geom_histogram(aes(x=weather[,i]),stat="count")+
    ggtitle(colnames(weather)[i])
  print(plot)
}
```
